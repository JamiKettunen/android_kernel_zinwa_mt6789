...
#define upm6922_info(fmt, ...) \
	do { \
		if (dbg_log_en) \
			pr_err(fmt, ##__VA_ARGS__); \
	} while (0)

#define upm6922_err(fmt, ...) \
	do { \
		if (dbg_log_en) \
			pr_err(fmt, ##__VA_ARGS__); \
	} while (0)
...
enum upm6922_adc_chan {
	ADC_CHAN_VBUS,
	ADC_CHAN_VBAT,
	ADC_CHAN_NOT_SUPPORT,
	ADC_CHAN_VSYS,
	ADC_CHAN_IBAT,
	ADC_CHAN_IBUS,
	ADC_CHAN_MAX,
