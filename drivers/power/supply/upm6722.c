...
*/
#define pr_fmt(fmt)	"[upm6722] %s: " fmt, __func__
...
static int upm6722_read_byte(struct upm6722 *upm, u8 reg, u8 *data)
{
    int ret;
...
/*********************************************************************/
static int upm6722_enable_charge(struct upm6722 *upm, bool enable)
{	
    u8 val;
...
static int upm6722_check_charge_enabled(struct upm6722 *upm, bool *enable)
{
    int ret = 0;
    u8 val = 0, val1 = 0;
...
static int upm6722_enable_batocpalm(struct upm6722 *upm, bool enable)
{
    u8 val;

    if (enable)
        val = UPM6722_BAT_OCP_ALM_ENABLE;
    else
        val = UPM6722_BAT_OCP_ALM_DISABLE;

    val <<= UPM6722_BAT_OCP_ALM_DIS_SHIFT;

    return upm6722_update_bits(upm, UPM6722_REG_03,
                UPM6722_BAT_OCP_ALM_DIS_MASK, val);
}
...
static int upm6722_get_adc_data(struct upm6722 *upm, adc_channel_t channel,  int *result)
{
    int ret = 0;
    u8 val_l = 0, val_h = 0;
    s16 val = 0;
	...
	if (channel == UPM_ADC_IBAT && ibat_use_pmu) {
		psy = power_supply_get_by_name("mtk-master-charger");
		if (psy == NULL) {
			upm_err("get master charger psy fail!\n");
			goto out;
		}
			
		info = (struct mtk_charger *)power_supply_get_drvdata(psy);
		if (info == NULL) {
			upm_err("get mtk_charger info fail!\n");
			goto out;
		}

		sval_pmu = get_battery_current(info);
		if(sval_pmu < 0)
			sval_pmu = 10;
	}
	...
	if (channel == UPM_ADC_IBAT && ibat_use_pmu) {
		*result = sval_pmu;
		upm_info("channel:%d, val_h:0x%x, val_l:0x%x, sval_pmu:%d,sval:%d\n",
		channel, val_h, val_l, sval_pmu,sval);
	} else {
		*result = sval;
...
static int upm6722_init_protection(struct upm6722 *upm)
{
    ...
    ret = upm6722_enable_batocpalm(upm, !upm->cfg->bat_ocp_alm_disable);
...
static int upm6722_set_present(struct upm6722 *upm, bool present)
{
...
static int upm6722_psy_register(struct upm6722 *upm)
{
    ...
    if (upm->mode == UPM6722_ROLE_MASTER)
        ...
        upm->psy_desc.name = "upm6722-standalone";
...
static void upm6722_check_alarm_fault_status(struct upm6722 *upm)
{
    int ret, i;
    u8 val = 0;
                    ...
                    const struct i2c_device_id *id)
{
    ...
    upm = devm_kzalloc(&client->dev, sizeof(struct upm6722), GFP_KERNEL);
    if (!upm) {
