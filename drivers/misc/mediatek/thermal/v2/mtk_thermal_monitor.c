...
#define THRML_ERROR_LOG(fmt, args...) pr_info("THERMAL/MONITOR " fmt, ##args)
...
int tc1, int tc2, int passive_delay, int polling_delay)
{
	...
	if (ret) {
		...
		pr_info("%s %s thermal_zone_device_enable fail, ret=%d\n", __func__, type, ret);
		return tz;
	}
...
static int  thermal_monitor_init(void)
{
		mtkthermal_init();
...
static void thermal_monitor_exit(void)
{
	mtkthermal_exit();
