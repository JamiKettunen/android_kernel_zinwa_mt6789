...
/*
 * linux version before 6.4.0 use non-static
 * argument for class_find_device()
 */
...
static int device_match_name(struct device *device, const void *devname)
{
	...
	return !strncmp(devname, tee->name, sizeof(tee->name));
}
	...
	struct tee_shm_io __user *u_shm)
{
	...
	if ((k_shm.buffer == NULL) || (k_shm.fd_shm != 0)) {
		pr_err("Invalid shm param buffer/fd\n");
		goto exit;
	}

	update_shm_size = k_shm.size == 0;
	...
	const struct tee_ops *ops, size_t len)
{
	...
	/*
	 * tee->shm_flags: acceptable flags for shared memory
	 * add support of TEEC_MEM_NAMEDSHM here
	 */
	tee->shm_flags = TEEC_MEM_INPUT | TEEC_MEM_OUTPUT |
		TEEC_MEM_NONSECURE | TEEC_MEM_NAMEDSHM;
