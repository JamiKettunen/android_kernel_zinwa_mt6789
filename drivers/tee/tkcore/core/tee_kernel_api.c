...
#else
#define TEE_TZ_DEVICE_NAME	"tkcore_client"
#endif
	...
	struct TEEC_SharedMemory *shared_memory)
{
	...
	shm = (struct tee_shm *) shm_io.tee_shm;

	shared_memory->d.ptr = shm;
	shared_memory->buffer = shm->static_shm.kaddr;
...
void TEEC_ReleaseSharedMemory(struct TEEC_SharedMemory *shared_memory)
{
	...
	if (WARN_ON(!(shared_memory->flags & TEEC_MEM_KAPI)))
		return;

	shm = (struct tee_shm *) shared_memory->d.ptr;
	tee_shm_free_io(shm);

	shared_memory->buffer = NULL;
	shared_memory->d.ptr = NULL;
}
