					...
					size_t count)
{
	...
	while ((len != count) && ((ulog->rseq != wseq) ||
		...
		if (ulog->tmpbuf) {
			...
			if (copy_len == n) {
				pr_warn("failed to copy flag to user");
				return len;
			}
			...
			n = copy_to_user(&buf[len],
				...
				copy_len);
			if (copy_len == n) {
				pr_warn("failed to copy klog to user\n");
				return len;
			}
				...
				size_t count, loff_t *pos)
{
	...
	if (ulog == NULL) {
		pr_warn("file not open correctly\n");
		return -EINVAL;
	}
...
static int tee_log_open(struct inode *inode, struct file *file)
{
	int ret;
	struct ulog *ulog;
	...
	if (unlikely(ret)) {
		...
		pr_warn("open file failed with %d\n", ret);
		return ret;
	}
...
static int register_klog_irq(struct tee *tee, struct klog *klog)
{
	...
	if (tee->log.irq == 0) {
		pr_warn("unknown tee log irq\n");
		return 0;
	}

	pr_info("tee log irq = %d\n", tee->log.irq);
	...
	if (r != 0) {
		pr_err("failed to register klog_irq with %d\n", r);
		return -1;
	}
						...
						size_t shm_len)
{
	...
	if (shm_len <= TEE_LOG_CTL_BUF_SIZE) {
		pr_err("init_klog_shm: shm_len %zu too small\n", shm_len);
		return -1;
	}
	...
	if (rb_len != shm_len - TEE_LOG_CTL_BUF_SIZE) {
		pr_err("Unexpected shm length: %u\n", rb_len);
		return -1;
	}
...
int tkcore_notify_logger(bool flush)
{
	wake_up_all(&klog.wq);
	if (!flush)
		return 0;

	return wait_event_interruptible(klog.flush_wq,
		READ_ONCE(klog.log_ctl->info.tee_read_seq) ==
			READ_ONCE(klog.log_ctl->info.tee_write_seq));
}
EXPORT_SYMBOL(tkcore_notify_logger);
...
static int init_klog(struct klog *klog, struct tee *tee)
{
	...
	init_waitqueue_head(&klog->flush_wq);
