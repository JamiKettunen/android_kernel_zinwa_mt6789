...
static int evdi_queue_event(struct evdi_device *evdi,
                            ...
                            int poll_id)
{
    ...
    event = evdi_event_alloc(evdi, type, poll_id,
                             (void *)payload, payload_size, owner);
    if (!event)
        return -ENOMEM;
                                          ...
                                          struct drm_file *owner)
{
    ...
    return evdi_queue_event(evdi, type, payload,
                            payload_size, owner, poll_id);
}
...
static void evdi_inflight_cancel(struct evdi_device *evdi,
...
{
    struct evdi_gralloc_data *gralloc = req->reply.get_buf.gralloc_buf.gralloc;
    int fd, i;
    ...
    if ((fd = evdi_get_unused_fds_batch(gralloc_buf->numFds, O_RDWR, installed_fds)) < 0) {
...
static int evdi_copy_user_allow_partial(void __user *dst, const void *src, size_t len, bool to_user)
{
    size_t not;
    ...
    not = to_user ? copy_to_user(dst, src, len) : copy_from_user(dst, src, len);
    return (not == len) ? -EFAULT : 0;
}
...
static int evdi_queue_create_event_with_id(struct evdi_device *evdi,
        ...
        int poll_id)
{
    return evdi_queue_event(evdi, create_buf,
...
static int evdi_queue_get_buf_event_with_id(struct evdi_device *evdi,
        ...
        int poll_id)
{
    return evdi_queue_event(evdi, get_buf,
                            params, sizeof(*params),
                            owner, poll_id);
}
...
deliver:
    ...
    if (payload_size && cmd->data) {
        if (evdi_copy_user_allow_partial(cmd->data,
        								payload_buf,
        								payload_size,
        								true)) {
...
int evdi_ioctl_gbm_get_buff(struct drm_device *dev, void *data, struct drm_file *file)
{
	...
	if (evdi_copy_user_allow_partial(cmd->native_handle, gralloc_buf, copy_size, true)) {
		for (i = 0; i < gralloc_buf->numFds; i++)
...
int evdi_ioctl_gbm_create_buff(struct drm_device *dev, void *data, struct drm_file *file)
{
	...
	if (u_id) {
		if (evdi_copy_user_allow_partial(u_id, &req->reply.create.id, sizeof(*u_id), true)) {
			evdi_inflight_req_put(req);
			return -EFAULT;
		}
	}
	...
	if (u_stride) {
		if (evdi_copy_user_allow_partial(u_stride, &req->reply.create.stride, sizeof(*u_stride), true)) {
			evdi_inflight_req_put(req);
			return -EFAULT;
		}
	}
...
int evdi_ioctl_get_buff_callback(struct drm_device *dev, void *data, struct drm_file *file)
{
	...
	if (nint) {
		if (evdi_copy_user_allow_partial(gralloc->data_ints,
						 cb->data_ints,
						 sizeof(int) * nint,
						 false))
			goto fail_gralloc;
	...
	if (nfd) {
		if (evdi_copy_user_allow_partial(fds_local, cb->fd_ints,
						 sizeof(int) * nfd, false))
			goto fail_gralloc;
...
static int evdi_queue_int_event(struct evdi_device *evdi,
        ...
        struct drm_file *owner)
{
    return evdi_queue_event_autoid(evdi, type,
                                   &v, sizeof(v),
                                   owner);
}

int evdi_queue_swap_event(struct evdi_device *evdi,
        int id, int display_id,
        struct drm_file *owner)
{
    ...
    if (evdi_queue_event_autoid(evdi, swap_to,
                                &sw, sizeof(sw),
                                owner))
        return -ENOMEM;
...
int evdi_queue_create_event(struct evdi_device *evdi,
               ...
               struct drm_file *owner)
{
    return evdi_queue_event_autoid(evdi, create_buf,
                                   params, sizeof(*params),
                                   owner);
}
